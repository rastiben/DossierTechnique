$(document).ready(function () {
        ...
        , success: function (data) {
            ...
            $.each($json, function ($number, $obj) {
                ...
                if ($obj["idPhoto"] != 0) {
                    ...
                    $(".row.notes").append('<div class="col-md-12 col-md-offset-3"><div class="col-md-6 divNote">' +
                        $important +
                        '<h2 class="col-md-12">' + $obj["client"] + '</h2>' +
                        '<p class="col-md-12 note">' + $obj["note"] + '</p>' +
                        '<img class="col-md-12" id="img' + $number + '"></img>' +
                        '<div class="col-md-12 text-right"><a class="btn btn-default" id="' + $number + '"><span class="glyphicon glyphicon-pencil"></span>Modifier</a></div>' +
                        '<div class="col-md-12"><hr></div>' +
                        '<p class="col-md-8" >' + $obj["Tech"] + '</p>' +
                        '<p class="col-md-4 text-right date">' + $obj["noteDate"].split(" ")[0] + '</p>' +
                        '</div></div>')
                    ...
                    $(".row.notes").append('<div class="col-md-12 col-md-offset-3"><div class="col-md-6 divNote">' +
                        $important +
                        '<h2 class="col-md-12">' + $obj["client"] + '</h2>' +
                        '<p class="col-md-12 note" id="' + $obj["_id"] + '">' + $obj["note"] + '</p>' +
                        '<div class="col-md-12 text-right"><a class="btn btn-default" id="' + $number + '"><span class="glyphicon glyphicon-pencil"></span>Modifier</a></div>' +
                        '<div class="col-md-12"><hr></div>' +
                        '<p class="col-md-8" >' + $obj["Tech"] + '</p>' +
                        '<p class="col-md-4 text-right date">' + $obj["noteDate"].split(" ")[0] + '</p>' +
    ...
    $(document).on("click", ".btn.btn-default", function () {
        ...
        bootbox.dialog({
            ...
            , message: '<div class="row">  ' +
                ...
                '<textarea class="form-control modification" id="' + notes[$(this).attr('id')]["_id"] + '" contentEditable>' + notes[$(this).attr('id')]["note"] + '</textarea> ' +
                '</form> </div>  </div>'
            , buttons: {
                success: {
                    label: "Save"
    ...
    $(".buttons .btn-success").click(function () {
        ...
        if (clientNewNote != null && techNewNote != null && note != "") {
            ...
            if (pictureBase64NewNote == "") {
                $(".row.notes").prepend($('<div class="col-md-12 col-md-offset-3"><div class="col-md-6 divNote">' +
                    $important +
                    '<h2 class="col-md-12">' + clientNewNote["name"] + '</h2>' +
                    '<p class="col-md-12 note">' + note + '</p>' +
                    '<div class="col-md-12"><hr></div>' +
                    '<p class="col-md-8" >' + techNewNote["name"] + '</p>' +
                    '<p class="col-md-4 text-right date">' + date + '</p>' +
                    '</div></div>').fadeIn("slow"));
            } else {
                $(".row.notes").prepend($('<div class="col-md-12 col-md-offset-3"><div class="col-md-6 divNote">' +
                    $important +
                    '<h2 class="col-md-12">' + clientNewNote["name"] + '</h2>' +
                    '<p class="col-md-12 note">' + note + '</p>' +
                    '<img class="col-md-12" src="data:image/png;base64,' + pictureBase64NewNote + '"></img>' +
                    '<div class="col-md-12"><hr></div>' +
                    '<p class="col-md-8" >' + techNewNote["name"] + '</p>' +
                    '<p class="col-md-4 text-right date">' + date + '</p>' +
                    '</div></div>').fadeIn("slow"));
